...
class OtherWidget(QWidget, Ui_Form):
    ...
    def initWidget(self):
        ...
        menu = RoundMenu(parent=widget_pipreqs)
        ...
        menu = RoundMenu(parent=widget_generate_code)
        ...
        if self.comboBox_mode.currentText() == "独立模式":
            ...
            if CURRENT_SETTINGS["settings"]["mode"] == "现有环境":
                ...
                if not CURRENT_SETTINGS["settings"]["pyenv_venv_current_version"]:
                    ...
                    return
                path = os.path.join(CURRENT_SETTINGS["settings"]["pyenv_path"], "envs", CURRENT_SETTINGS["settings"]["pyenv_venv_current_version"], "Scripts",
                                    "python.exe")
            else:
                pass
            ...
            if CURRENT_SETTINGS["project"]["mode"] == "独立模式":
                ...
                if CURRENT_SETTINGS["settings"]["mode"] == "现有环境":
                    ...
                    if not CURRENT_SETTINGS["settings"]["pyenv_venv_current_version"]:
                        Message.error("错误", "请设置Pyenv-venv环境", self)
                        return
                    path = os.path.join(CURRENT_SETTINGS["settings"]["pyenv_path"], "envs",
                                        CURRENT_SETTINGS["settings"]["pyenv_venv_current_version"], "Scripts", "python.exe")
                else:
                    pass
        return path
